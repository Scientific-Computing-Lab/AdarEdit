...
1) bedtools sort
2) bedtools merge -d <distance> [-s] -c 3,3 -o count,collapse
...
- To emit the alternate column order ($1,$2,$3,$5,$6,$4), use
  --output-mode compat (not BED6-valid).
"""
...
def cluster_sites(
    ...
    """
    Run: bedtools sort | bedtools merge -d D [-s] -c 3,3 -o count,collapse
    ...
    # With -s, bedtools emits strand as column 4. Requesting column 6 again
    # would duplicate strand and shift the count/collapsed-site fields.
    merge_cmd = ["bedtools", "merge", "-d", str(distance)]
    if strand_specific:
        merge_cmd.append("-s")  # place -s AFTER the distance
    merge_cmd += ["-c", "3,3", "-o", "count,collapse"]
    ...
    with output_bed.open("w", encoding="utf-8") as fout:
        ...
        for line in merge_proc.stdout:
            total += 1
            parts = line.rstrip("\n").split("\t")
            # Strand-specific output: chr, start, end, strand, count, collapsed.
            # Non-strand-specific output: chr, start, end, count, collapsed.
            expected = 6 if strand_specific else 5
            if len(parts) < expected:
                # Skip malformed rows
                continue
            chrom, start, end = parts[0], parts[1], parts[2]
            if strand_specific:
                strand, count_str, collapsed = parts[3], parts[4], parts[5]
            else:
                count_str, collapsed = parts[3], parts[4]
                strand = "."
