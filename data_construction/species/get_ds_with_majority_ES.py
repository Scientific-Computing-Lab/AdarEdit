...
def run_bprna(dbn_file: str, output_dir: str, bprna_path: str) -> str:
    perl = shutil.which("perl") or "perl"
    base = os.path.splitext(dbn_file)[0] + ".st"
    out_st = os.path.join(output_dir, os.path.basename(base))
    subprocess.run([perl, bprna_path, os.path.abspath(dbn_file)], cwd=output_dir, check=True)
    if not os.path.isfile(out_st):
        raise RuntimeError(f"bpRNA did not create the expected output: {out_st}")
    return out_st
...
def get_genome_position(
    ...
    """Map one-based inclusive sequence coordinates to BED half-open intervals."""
    fold_region_start, fold_region_end = fold_region
    start_1ds, end_1ds, start_2ds, end_2ds = coords

    if strand == "+":
        s1g = fold_region_start + start_1ds - 1
        e1g = fold_region_start + end_1ds
        s2g = fold_region_start + start_2ds - 1
        e2g = fold_region_start + end_2ds
    else:
        s1g = fold_region_end - end_1ds
        e1g = fold_region_end - start_1ds + 1
        s2g = fold_region_end - end_2ds
        e2g = fold_region_end - start_2ds + 1
...
def parse_position_id(position_id: str) -> Tuple[int, int] | None:
    ...
    # Chromosome/scaffold identifiers may themselves contain numeric tokens.
    # The interval is always stored in the final two underscore-separated fields.
    parts = position_id.rsplit("_", 2)[-2:]
    if len(parts) >= 2:
        try:
...
def intersect_with_all_site(
    ...
    def run_intersect(s: int, e: int) -> List[str]:
        bed_line = f"{chrom}\t{s}\t{e}\t0\t0\t{strand}"
...
def get_relative_site(
    ...
    """Compute a one-based local index in transcript-oriented ds1 + link + ds2.

    Genomic positions are one-based; segment intervals are BED-style, zero-based
    and half-open. The two sequence fragments are already oriented 5' to 3'.
    """
    ds_1 = start_1ds_genome < position <= end_1ds_genome
    ds_2 = start_2ds_genome < position <= end_2ds_genome
    if not ds_1 and not ds_2:
        raise ValueError(
            f"Genomic position {position} is outside both dsRNA arms: "
            f"[{start_1ds_genome},{end_1ds_genome}), "
            f"[{start_2ds_genome},{end_2ds_genome})"
        )
    ...
    if strand == "+":
        if ds_1:
            ...
            rel = (end_1ds_genome - start_1ds_genome) + len(link) + (position - start_2ds_genome)
    else:
        if ds_1:
            rel = end_1ds_genome - position + 1
        else:
            rel = (end_1ds_genome - start_1ds_genome) + len(link) + (end_2ds_genome - position + 1)
    return rel
...
def get_a_positions_in_window(
    ...
    for i, nt in enumerate(sequence):
        genomic_pos = segment_start + i + 1 if strand == "+" else segment_end - i
...
def process_row(row, gr: genome_reader, all_chr_length, args, link: str):
    ...
    mfe_data = {
        "sequence_id": seq_id,
        "region_id": f"{chr1}:{start1}-{end1}:{strand}",
        "strand": strand,
    ...
    if fold_region is None:
        raise RuntimeError(f"Invalid fold-region identifier: {position_id}")

    if args.write_structure_plots:
    ...
    st_file = run_bprna(dbn_filename, args.output_dir, args.bprna)
    ...
    if len(near_seqs[0]) != end_1ds_genome - start_1ds_genome:
        raise RuntimeError("First dsRNA arm length does not match its genomic interval")
    if len(near_seqs[1]) != end_2ds_genome - start_2ds_genome:
        raise RuntimeError("Second dsRNA arm length does not match its genomic interval")
    ...
    a_pos_ds1 = get_a_positions_in_window(
        near_seqs[0], start_1ds_genome, end_1ds_genome, strand, edit_1, start_1ds_genome, end_1ds_genome, start_2ds_genome, end_2ds_genome, link
    )
    a_pos_ds2 = get_a_positions_in_window(
        near_seqs[1], start_2ds_genome, end_2ds_genome, strand, edit_2, start_1ds_genome, end_1ds_genome, start_2ds_genome, end_2ds_genome, link
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--bprna", default=os.environ.get("BPRNA_PL", "bpRNA.pl"),
        help="Path to bpRNA.pl (or set BPRNA_PL).",
    )
    parser.add_argument(
        "--write-structure-plots", action="store_true",
        help="Also create CT/SVG visualizations with RNAstructure dot2ct/draw.",
    )
    ...
    resolved_bprna = shutil.which(args.bprna) if os.path.basename(args.bprna) == args.bprna else args.bprna
    if not resolved_bprna or not os.path.isfile(resolved_bprna):
        parser.error("bpRNA.pl was not found; provide --bprna /path/to/bpRNA.pl or set BPRNA_PL")
    args.bprna = os.path.abspath(resolved_bprna)
    if args.write_structure_plots:
        for program in ("dot2ct", "draw"):
            if shutil.which(program) is None:
                parser.error(f"--write-structure-plots requires {program} in PATH")
