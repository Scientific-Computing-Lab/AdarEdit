#!/usr/bin/env python3
"""Train joint multi-label models with or without Combined."""
...
class JointBioAwareGNN(bio.BioAwareGNN):
    """Checkpoint-key-compatible bio-aware encoder with a configurable-width head."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--data-root", type=Path, default=PACKAGE_ROOT / "data/human"
    )
    parser.add_argument(
        "--cache-root", type=Path, default=PACKAGE_ROOT / "cache/joint"
    )
...
def main() -> None:
    ...
    summary = {
        ...
        "mode": "multilabel",
        "experiment": (
            "joint_graph_with_combined"
            if args.tissue_set == "with_combined"
            else "joint_graph_no_combined"
        ),
        "tissue_set": args.tissue_set,
        "tissues": tissues,
