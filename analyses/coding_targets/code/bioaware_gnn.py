...
- Node features: base one-hot, paired flag, relative position to target (normalized),
  target flag, optional neighbor one-hots, stem/loop geometry, pair compatibility/type.
- Edge features: learned edge-type embedding (sequence / canonical pair / wobble
  pair / other structural pair) plus scalar attributes (seq vs pair flag,
  sequence distance, geometry on both ends, pair compatibility, distance to
  target indices).
...
# Coarse fixed base-pair compatibility scores used by the model.
# These are not a thermodynamic nearest-neighbour energy calculation.
PAIR_COMPATIBILITY = {
    ("A", "U"): -2.1,
    ("U", "A"): -2.1,
    ("G", "C"): -3.4,
...
def classify_pair(b1: str, b2: str) -> int:
    ...
    if pair in WOBBLE:
        return WOBBLE_EDGE
    # A dot-bracket pair remains a pair edge even when the supplied sequence
    # does not form one of the canonical/wobble identities.  Using SEQ_EDGE
    # here would falsely label that structural edge as backbone connectivity.
    return OTHER_PAIR_EDGE
...
def true_stem_lengths(struct: str, pm: Dict[int, int] | None = None) -> List[int]:
    """Return the full contiguous nested-helix length for every paired node."""
    pm = paired_map(struct) if pm is None else pm
    stem_len = [0] * len(struct)
    unique_pairs = {(min(i, j), max(i, j)) for i, j in pm.items()}
    for left, right in unique_pairs:
        length = 1
        outer_left, outer_right = left - 1, right + 1
        while (outer_left, outer_right) in unique_pairs:
            length += 1
            outer_left -= 1
            outer_right += 1
        inner_left, inner_right = left + 1, right - 1
        while (inner_left, inner_right) in unique_pairs:
            length += 1
            inner_left += 1
            inner_right -= 1
        stem_len[left] = length
        stem_len[right] = length
    return stem_len
...
def stem_loop_geometry(struct: str) -> Tuple[List[int], List[int], List[int]]:
    ...
    stem_len = true_stem_lengths(struct, pm)
    loop_len = [0] * n
    dist_junc = [0] * n
...
def build_graph(ex: GraphExample, use_neighbors: bool, use_geometry: bool, plfold_dir: str = None, idx: int = None) -> Data:
    ...
    for i, base in enumerate(ex.seq):
        ...
        pair_compatibility = (
            PAIR_COMPATIBILITY.get((base, ex.seq[pm[i]].upper()), 0.0)
            if i in pm
            else 0.0
        )
        feats += [pair_compatibility]
    ...
    def add_edge(i: int, j: int, et: int, is_seq: float, dist_norm: float):
        ...
        pair_compatibility = (
            0.0
            if is_seq
            else PAIR_COMPATIBILITY.get(
                (ex.seq[i].upper(), ex.seq[j].upper()), 0.0
            )
        )
        edge_scalar.append(
            [
    ...
    for i, j in sorted((i, j) for i, j in pm.items() if i < j):
    ...
    return Data(
        ...
        stem_raw=torch.tensor(stem_len, dtype=torch.long),
    )
...
class BioAwareGNN(nn.Module):
    def __init__(
        ...
        self.edge_emb = nn.Embedding(4, edge_emb_dim)
